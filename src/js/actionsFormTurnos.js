...
areaSelect.addEventListener("change", () => {
  ...
  const diasServicio = APPOINTMENT_SCHEDULE.filter((d) => d.area === areaSeleccionada);
  ...
  diasServicio.forEach((d) => {
    ...
    if (d === diasServicio[0]) {
      ...
      const horasDiaCorrespondiente = APPOINTMENT_SCHEDULE.filter((h) => h.dia === d.dia);
      ...
      horasDiaCorrespondiente.forEach((h) => {
        ...
        h.hora5 && horaSelect.append(new Option(h.hora5, h.hora5));
        h.hora6 && horaSelect.append(new Option(h.hora6, h.hora6));
      });
    }
  });
});
